...
int main()
{
    ...
    for(i = 0; i < quantAlunos; i++)
    {
        printf("%05d       %-18s  %-8.2lf %-8.2lf %-8.2lf ", infoAlunos[i].matricula, infoAlunos[i].nome, infoAlunos[i].nota1, infoAlunos[i].nota2, infoAlunos[i].nota3);
        if( RetornaMedia(infoAlunos[i]) >= 4 && RetornaMedia(infoAlunos[i]) < 7 ) /// caso de aluno em recuperacao
        {
            printf("%-7.2lf %-9.2lf", infoAlunos[i].notaRec, RetornaMediaFinal(infoAlunos[i]));
        ...
        {
            printf("-       %-9.2lf", RetornaMedia(infoAlunos[i]));
